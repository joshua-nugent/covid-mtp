...
day <- "results/tvcovar_2021_10_23"
leadY <- "2wks"
WY_lag <- "lag1"
...
absplot <- function(data,
  ...
  geom_errorbar(data = a1 %>% filter(adj_type == "Adjusted"),
    ...
    labs(title = paste0(a1$exposure[1], ", allowing for time-varying confounder set"),
...
ggsave(filename = "_da_tv.png",
       ...
       plot = theplot,
       bg = "white")
(theplot <- absplot(data = abc2 %>% filter(exposure == "residential_percent_change_from_baseline"), y_range = casey, factor_reverse = F))

ggsave(filename = "_gh_tv.png",
       device = "png",
       dpi = 320,
       width = 6,
       height = 3,
