...
lmtp_analysis <- function(
  ...
  if(lagA){
    ...
    la <- the_data %>% ungroup() %>%
      filter(week == (wk - 2)) %>%
      select(all_of(c(exposure, "fips"))) %>% 
      rename(lagA = exposure)
    the_data <- full_join(the_data, la, by = c("fips"))
  }
  
  plan(multisession, workers = cores)
  dat <- the_data %>%
    select(all_of(c(Wvars,
                    outcome,
                    exposure,
                    "week", "state_abb", "fips",
                    "cum_cases_this_date"))) %>%
...
analysis_wrapper <- function(covars,
  ...
  for(i in 1:nrow(grid_matrix)){
    ...
    if(grid_matrix[i,"pop"] == "high"){
      ...
      print("================================================================")
      print("analyzing low-population counties now")
      data_pd <- data %>% filter(population < pop_cutoff)
    } else {
      data_pd <- data
      print("================================================================")
      print("not stratifying by population")
    }
